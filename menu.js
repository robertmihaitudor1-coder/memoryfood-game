...
function flip(card){
  ...
  if (isMatch){
    ...
    if (mode === "single") score += 10;
      ...
      // match => jucătorul mai joacă o dată
    }
...
function startGame(){
  ...
  if (mode === "multi"){
    // în multiplayer folosim hard by default (8x6)
    diff = "hard";
  }

  const cfgFinal = DIFFICULTIES[diff];
  totalPairs = cfgFinal.pairs;

  setBoardGrid(cfgFinal.cols);

  // reset scores
  moves = 0; score = 0;
  p1 = 0; p2 = 0; turn = 1;

  const deck = buildDeck(cfgFinal.pairs);
  boardEl.innerHTML = "";
